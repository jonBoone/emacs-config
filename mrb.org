...
#+name: extend-attach
#+begin_src emacs-lisp :tangle no
  ;; FIXME: return info, do not use insert directly, so we have more control
  ...
  (use-package mml-sec
    ...
    (setq mml-secure-key-preferences
          '((OpenPGP
             (sign)
             (encrypt
              (user-mail-address user-gpg-encrypt-key)))
            (CMS
             (sign)
             (encrypt))))
...
#+begin_src emacs-lisp
  ;; FIXME: check for system packages meson, g++, pkg-config,
...
** Emacs Lisp
I've not done a lot of Emacs Lisp development, so I imagine this section might change frequently until I have a handle on which libraries and extensions to use.
...
#+begin_src emacs-lisp
  (use-package sly
    ...
    (defun mrb/determine-cmucl-version (binary version-params)
      ...
      (with-temp-buffer
        (let* ((invocation-return-value
                (call-process-shell-command binary nil (current-buffer)
                                            t version-params))
               (invocation-result
    ...
    (setq cmucl-path "/opt/cmucl"
          ...
          cmucl-version-params "-quiet -eval \"(prin1 (lisp:lisp-implementation-version)) (quit)\"")

    (setq cmucl-binaries (mrb/find-cmucl-binaries
                          cmucl-path cmucl-binary-regex))

    (setq cmucl-version-alist
          (mapcar (lambda (binary)
                    (cons
                     (mrb/determine-cmucl-version binary
                                                  cmucl-version-params)
                     binary))
                  cmucl-binaries))
