...
#+SUBTITLE: derived from [[https://github.com/mrvdb/emacs-config][Marcel van der Boom's Emacs Config]]
#+SUBTITLE: with additions from [[http://www.eastpyne.org/Emacs/fsem.html][Rob Rix's Hardcore Freestyle Emacs config]]
#+SUBTITLE: and further inspiration from[[https://github.com/lccambiaghi/vanilla-emacs][Luca Cambiaghi's Vanilla Emacs literate config]]
#+LANGUAGE: en
...
I am not entirely sure if I need the early-init.el file though.
...
And edit the file obviously. __Note: this script passes shellcheck with no warnings or errors__

#+begin_src sh :exports code :tangle ~/bin/emacs :shebang #!/bin/bash
  # <<tangle-header>>
  # Shell-script emacs that wraps emacs-client usage to handle specific cases:
  # - no filename given
  # - stdin editing
  # - force tty
...
#+begin_src emacs-lisp
  ...
  (setq use-package-always-ensure nil   ; Make sure this is nil, so we do not use package.el
        use-package-verbose 'debug      ; FIXME: use a debug var for all of config?
        )
  ;; From this point on we should be able to use `use-package
  (use-package straight
    :config
    (setq straight-host-usernames '((github . "jonBoone"))) ; Move to personal information?
  ...
  ;; Need to install dependencies of use-package manually
...
#+begin_src emacs-lisp
  (setq mrb/default-email-address "ipmonger@delamancha.org"
        mrb/default-signature-file "~/.signatures/ipmonger%40delamancha.org"
        mrb/private-addresses '("iain@alumni.cmu.edu" "ipmonger@gmail.com" "ipmonger@icloud.com" "reagent@elegant-software.io")
        mrb/private-signature-file "~/.signatures/.signature-private"
        mrb/default-private-address (car mrb/private-addresses)

        mrb/bcc-address      "ipmonger@delamancha.org"
        user-full-name       "Jon Allen Boone"
        user-mail-address    mrb/default-email-address
        user-domain          "delamancha.org"
        user-organisation    "N/A"
        user-gpg-encrypt-key "E57ED1E3B5A96490C4F544B425B64512B9B7A4F1"
        )
#+end_src
...
#+begin_src emacs-lisp
  (use-package all-the-icons
    :if (display-graphic-p))              ; FIXME: when in daemon mode, this does not get loaded
#+end_src
...
mrvdb used the [[https://github.com/arcticicestudio/nord][Nord color palette]]. This palette defines 16 colors, applying it to the base16 theme.  Since it is not to my taste, I have removed that code and replaced it with [[https://github.com/protesilaos/modus-themes][Modus Themes]].
...
#+begin_src emacs-lisp
  ...
  (use-package modus-themes
    ...
    :config
    ;; load the themes of your choice:
...
#+begin_src emacs-lisp
  ...
  (use-package spaceline
    :straight (:includes spaceline-config))
  ...
  ;; Use whitespace mode to cleanup excess whitespace
  (use-package whitespace
    :diminish (('whitespace-mode        . "ᗣ")
               ('global-whitespace-mode . "ᗣ"))
    :config
    ;; whitespace-style rules
    (setq whitespace-style '(indentation::space
                             ...
                             face
                             tabs))
    ;; whitespace-cleanup on all saves
    (add-hook 'before-save-hook 'whitespace-cleanup)
    ;; enable whitespace mode on text-mode and derivatives
    (defun mrb/setup-text-mode ()
      "apply all text-mode customizations"
      (whitespace-mode 1))
    (add-hook 'text-mode-hook 'mrb/setup-text-mode))
...
Column-numbering is off by default, but I want it on.
...
#+begin_src emacs-lisp
  (use-package flyspell
    ...
    ;; FIXME: this does not seem to help
...
3. perhaps some FIXME keywords in code, I use that quite a bit.
...
** Searching for items
Over the years the amount of information I manage with orgmode has grown considerably, ranging from the plain TODO lists, blog posts, calendar information, project files etc. Increasingly I find myself not being able to find things in that pile of information. The solution in [[Grepping files]] helps a lot already but is a generic solution.
...
#+name: org-journal-mode-selected
#+begin_src emacs-lisp :tangle no
    :init
...
If there is a need to open a **Completions** window, let's at least do it predictably, so I know what and where to expect it.  The =popwin= package make this behavior a bit more predictable by showing it in the same place and making sure I can get rid of it easily.
...
#+begin_src emacs-lisp
  (use-package helm
    ...
    (("C-c M-x" . 'execute-extended-command) ;; Save the default
     ("M-y"     . 'helm-show-kill-ring)      ;; display kill ring
     ("M-x"     . 'helm-M-x)                 ;; Replace M-x
     ("<menu>"  . 'helm-M-x)                 ;; ..and on the menu key as well
     ("C-h SPC" . 'helm-all-mark-rings)      ;; show mark rings
     ("C-:"     . 'helm-company)             ;; autocomplete with helm
...
#+begin_src emacs-lisp
  ...
  (use-package mastodon
    ...
    (setq mastodon-instance-url "https://hachyderm.io"
          ...
          ;; finds hachyderm.io in password-store and expects user field to be present
...
#+begin_src emacs-lisp
  (use-package lsp-mode
   ...
   :hook
   (((c++-mode c-mode css-mode html-mode js2-mode python-mode sh-mode yaml-mode go-mode)
    . lsp-deferred)
...
#+begin_src emacs-lisp
  (use-package tree-sitter
    :init
    (setq tsc-dyn-get-from '(:compilation)) ; dont fetch binaries, ppc64 likely is not there
    :config
    ;; Needs to be set before tree-sitter-langs is loaded
    (setq tree-sitter-langs-grammar-dir
          (expand-file-name "straight/repos/tree-sitter-langs/" user-emacs-directory))
...
#+begin_src emacs-lisp
  (use-package ts-fold
    ...
    :bind (("M-S-<iso-lefttab>" . ts-fold-toggle))
    :config
    ;; Enable folding when tree-sitter gets swtiched on
...
Using dash.el for enhanced list support in Emacs Lisp

#+begin_src emacs-lisp
  ;; dash.el provides an alternative API for lists in Elisp
  ;; see https://github.com/magnars/dash.el
  (use-package dash
    :init (global-dash-fontify-mode))
#+end_src

Using eros for overlays with results in line
#+begin_src emacs-lisp
  (use-package eros
...
Also be aggressive about keeping the buffer indented
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package go-mode
    ;; FIXME: godef is fairly heavy on my little machines, make optional?
    :ensure-system-package godef
    :config
    :init
    ;; Set up before-save hooks to format buffer and add/delete imports.
    ;; Make sure you don't have other gofmt/goimports hooks enabled.
    ;; FIXME: would this not add hooks for all files that are saved??
...
#+begin_src emacs-lisp
  (use-package paredit
    :hook ((lisp-interaction-mode emacs-lisp-mode
            eval-expression-mode scheme-mode
...
#+begin_src emacs-lisp
    (use-package sly
      :after (paredit)
      :hook ((sly-mrepl-mode . override-sly-repl-bindings-with-paredit)
             (sly-mrepl-mode . enable-paredit-mode)
             (sly-mrepl-mode . sly-mrepl-font-lock-setup))
      :commands (mrb/nyxt)
      :init
      ;; In the REPL sly takes backspace, I want paredit to have it
      ...
      :config
      (defun mrb/determine-cmucl-version (binary version-params)
        "invoke binary with version-params to retrieve version string"
        (with-temp-buffer
          (let* ((invocation-return-value
                  (call-process-shell-command binary nil (current-buffer) t version-params))
                 (invocation-result))
            (car (split-string-and-unquote (buffer-string)))))
        invocation-result)

      (defun mrb/symbolize-version (implementation version)
        "create a :keyword symbol bsed on the implementation name
  and version strings"
        (intern (concat ":"
                        (concat implementation
                                (replace-regexp-in-string
                                 "[[:blank]]" "-" ; replace horizontal space with hyphens
                                 (replace-regexp-in-string
                                  "[{}]" "" ; remove parens
                                  version))))))

      (defun mrb/find-cmucl-binaries (path regex)
        "find the CMUCL binaries stored in path"
        (directory-files-recursively path regex nil
                                     'file-executable-p t))

      (setq cmucl-path "/opt/cmucl"
            cmucl-binary-regex "^lisp$"
            cmucl-version-params '-quiet -eval \"(prin1 (lisp-implementation-version)) (quit)\"'
            cmucl-binaries (mrb/find-cmucl-binaries
                            cmucl-path cmucl-binary-regex)

            cmucl-versions
            (mapcar (lambda (binary)
                      (list (mrb/symbolize-version
                             "cmucl"
                             (mrb/determine-cmucl-version
                              binary cmucl-version-params))
                            (list binary)))
                    cmucl-binaries))

      ;; note: intentionally not setting inferior-lisp-program
      ;; sly will try to autodetect a lisp implementation, unless
      ;; invoked with a negative prefix argument (M-- M-x sly)
      ;; in which case it will allow a list selection to be made
      (setq sly-lisp-implementations (quote (cmucl-versions))
...
I'm using elpy for python IDE functionality.
...
Setting up blacken for python formattting.
...
py-isort for keeping imports in order
...
I'm using pyright (from MS) as the python LSP server.
...
#+begin_src emacs-lisp
  (use-package flycheck
    ...
    (setq flycheck-python-pylint-executable (executable-find "python")
          ...
          flycheck-python-flake8-executable (executable-find "python")
...
#+begin_src emacs-lisp
  ...
  ;;  :hook
  ;;  (python-mode . py-docformatter-enable-on-save)
  ;;  :config
  ;;  (setq py-docformatter-options '("--wrap-summaries=88" "--pre-summary-newline")))
#+end_src

# pyment (for numpy docstring style) and normal python docstring style
...
#+end_src
  ...
  DAP mode is used for python debugging.

#+begin_src emacs-lisp
    (use-package dap-mode
      :after lsp-mode
      :hook
      ((dap-session-created . (lambda (_arg)
...
#+begin_src emacs-lisp
  (use-package helm-ag
    :commands mrb/search
    :after helm
    :bind (("C-s-s" . mrb/search))
    :config
    (setq helm-ag-use-agignore t)         ; Use the .agignore file if it is there
    :init
    (defun mrb/search ()
      (interactive)
      ;; but do *not* use the vcs ignore mechanism
      (setq helm-ag-command-option "--skip-vcs-ignores")
...
# Local Variables:
# eval: (setq last_updated (progn (org-babel-goto-named-src-block "last_updated") (org-babel-execute-src-block) (outline-hide-sublevels 1)))
# End:
