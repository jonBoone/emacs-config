...
#+TITLE: My org-babel based minimal emacs configuration for org-mode and org-roam support
#+LANGUAGE: en
...
Next to this main configuration file (=iain.org=), two other files are involved in the configuration:

1. =early-init.el= emacs' early initialization file;
   #+INCLUDE: early-init.el src emacs-lisp

2. =init.el= the main init file, which is used to generate =iain.el= from =iain.org=.
...
The config file =iain.org= gets opened *a lot* because I tend to fiddle with this config at least once a day. So, it warrants it's own keybinding, but it will have to wait until [[Key bindings]] before we can use the =bind-key= package.

#+begin_src emacs-lisp
  (defun iain/open-config ()
    (interactive)
...
#+begin_src emacs-lisp
  (setq iain/default-email-address "jon.boone@viasat.com"
        iain/private-addresses '("ipmonger@delamancha.org")
        user-full-name       "Jon Boone"
        user-mail-address    iain/default-email-address
        user-domain          "viasat.com"
        user-organisation    "Viasat"
        )
#+end_src
...
#+begin_src emacs-lisp
  (defun iain/infer-indentation-style ()
...
#+begin_src emacs-lisp
  ...
  (use-package autorevert
    ...
    :custom
    (auto-revert-interval 0.5)               ; Perhaps too often, let's see
  ...
  (defun iain/shortenurl-at-point ()
    (interactive)
  ...
  (bind-key "C-c s" 'iain/shortenurl-at-point)
#+end_src
...
#+begin_src emacs-lisp
  (use-package pdf-tools
    ...
    :commands (pdf-info-getannots iain/pdf-extract-info)
    :bind ( :map pdf-view-mode-map
            ...
            ("m"   . 'iain/mailfile)
            ("q"   . 'kill-this-buffer)
            ([remap pdf-misc-print-document] . 'iain/pdf-misc-print-pages)
    ...
    (defun iain/pdf-misc-print-pages(filename pages &optional interactive-p)
...
#+begin_src emacs-lisp
  ...
  (defun iain/set-default-input-method()
    (interactive)
    (with-temp-buffer
  ...
  (add-hook 'set-language-environment-hook 'iain/set-default-input-method)
  ...
  (add-hook 'text-mode-hook 'iain/set-default-input-method)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun iain/unicode-font-corrections()
    (interactive)
...
#+begin_src emacs-lisp
  (setq iain/cursor-type 'box)
  (setq iain/cursor-color "DarkOrange")  ; in fat orange color
  ...
  (setq default-frame-alist
        `((cursor-type . ,iain/cursor-type )
          (height . 60) (width . 100)
          (cursor-color . ,iain/cursor-color)
  ...
  (use-package mixed-pitch
    ...
    :config
    (set-face-attribute 'default t :font "FiraCode Nerd Font Mono" :height 160)
    (set-face-attribute 'fixed-pitch nil :font "FiraCode Nerd Font Mono")
    (set-face-attribute 'variable-pitch nil :font "Helvetica Neue"))
  (add-hook 'mixed-pitch-mode-hook #'solaire-mode-reset)n

  ;; proportional font pop-up frame support
...
#+begin_src emacs-lisp
    ...
    (use-package nano-modeline
      ...
      (tab-bar-format '(iain/tab-bar-format-primary
                        tab-bar-format-align-right
                        iain/tab-bar-format-secondary))
      ...
      (defun iain/tab-bar-format-primary () "") ;not used yet
      (defun iain/tab-bar-format-secondary ()
        (concat
  ...
  (use-package base16-theme
    ...
    (setq base16-adjustments
          `((vterm-color-black :foreground "#2e3440")
            ...
            (isearch                        :foreground "#434c5e" :background ,iain/cursor-color)
            ...
            (iain/org-todo-keyword-TODO      :foreground "#88c0d0" :weight bold)
            (iain/org-todo-keyword-DONE      :foreground "#a3be8c" :weight bold)
            (iain/org-todo-keyword-WAITING   :foreground "#bf616a" :weight bold)
            (iain/org-todo-keyword-CANCELLED :foreground "#3b4252" :weight bold)
            (iain/org-todo-keyword-BUY       :foreground "#d08770" :weight bold)
            (iain/org-todo-keyword-HOWTO     :foreground "#5e81ac" :weight bold)
            (iain/org-todo-keyword-INFO      :foreground "#ebcb8b" :weight bold)
            (iain/org-todo-keyword-COLLECT   :foreground "#a3be8c" :weight bold)
            (iain/org-todo-keyword-SOLVE     :foreground "#8fbcbb" :weight bold)
            (iain/org-todo-keyword-READ      :foreground "#b48ead" :weight bold)
            (iain/org-todo-keyword-READING   :foreground "#bf616a" :weight bold)
            (iain/org-todo-keyword-PLAN      :foreground "#d8dee9" :weight bold)
...
#+begin_src emacs-lisp
  (defun iain/run-client-settings(&optional args)
    (interactive)
  ...
  (add-hook 'server-after-make-frame-hook 'iain/run-client-settings)
  ;; And we need to run those settings *now* too, when we are not in server mode
  (iain/run-client-settings)
#+end_src
...
2. perhaps some TODO keywords in code, I use that quite a bit.
...
#+begin_src emacs-lisp
  ...
  (use-package svg-tag-mode
    ...
    (add-hook 'org-agenda-finalize-hook #'iain/org-agenda-show-svg))
#+end_src
* Buffers and files
...
#+begin_src emacs-lisp
   (setq auto-save-default t
         iain/auto-save-folder (concat user-emacs-directory "auto-save-list/"))

   (add-to-list 'auto-save-file-name-transforms
                (list "\\(.+/\\)*\\(.*?\\)" (expand-file-name "\\2" iain/auto-save-folder))
                t)
#+end_src
...
#+begin_src emacs-lisp
  (defun iain/move-file (new-location)
  ...
  (bind-key "C-x C-w" 'iain/move-file)
...
#+begin_src emacs-lisp
  (use-package dired
    ...
    :config
    (put
     'dired-find-alternate-file 'disabled nil)                 ; open dired in same buffer
    (setq
     dired-listing-switches "-agho --group-directories-first"  ; sort dired buffers
     dired-dwim-target t)                                      ; copy and move files between dired buffers
...
#+begin_src emacs-lisp
  (defun iain/buffer-majormode (&optional buffer-or-name)
...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    :bind (("C-c g"   . 'iain/gtd)
           ...
           :map org-mode-map
           ("s-."     . 'org-todo)
           ("s->"     . 'iain/org-cancel-todo)
           ("C-s-."   . 'iain/force-org-todo)
           ("M-p"     . 'org-set-property)
           :map org-agenda-mode-map
           ("s-."   . 'org-agenda-todo)
           ("s->"   . 'iain/org-agenda-cancel-todo)
           ("C-s-." . 'iain/force-org-agenda-todo)
...
#+begin_src emacs-lisp :tangle no
  (setq org-directory "~/Dropbox/pkb/org/"
...
#+begin_src emacs-lisp :tangle no
    (setq
     org-capture-templates
     '(("b" "Buy"
        entry (function iain/capture-location) "* BUY %? :buy:\n%(iain/prop-or-nothing :annotation)\n" :prepend t :empty-lines 1)
       ("l" "Link"
        entry (function iain/capture-location) "* TODO %?\n%(iain/prop-or-nothing :annotation)\n" :prepend t :empty-lines 1)
       ("t" "Todo"
        entry (function iain/capture-location) "* TODO %?\n" :prepend t :empty-lines 1)))


    (defun iain/prop-or-nothing(prop)
      "Insert `[:type]: [:property]` when :property has a value, otherwise nothing.
    Meant to be used inside org-capture-templates

    Example: `%(iain/prop-or-nothing :annotation)`"
...
#+begin_src emacs-lisp :tangle no
  :init
  (defun iain/capture-todo ()
    "Capture a TODO item"
    (interactive)
    (org-capture nil "t"))

  (defun iain/capture-buy ()
    "Capture a BUY item"
    (interactive)
    (org-capture nil "b"))

  (defun iain/capture-link ()
...
edit-noframe --eval '(iain/capture-todo)'
#+end_src

#+begin_src sh :exports code :tangle ~/bin/capture-buy.sh :shebang #!/bin/sh
edit-noframe --eval '(iain/capture-buy)'
#+end_src
...
#+begin_src emacs-lisp :tangle no
  :bind ( :map org-capture-mode-map
          ("C-c C-t" . iain/add-tags-in-capture))

  :init
  (defun iain/add-tags-in-capture()
    (interactive)
...
#+begin_src emacs-lisp :tangle no
  ...
  (defface iain/org-todo-keyword-TODO      nil "")
  (defface iain/org-todo-keyword-DONE      nil "")
  (defface iain/org-todo-keyword-WAITING   nil "")
  (defface iain/org-todo-keyword-CANCELLED nil "")
  (defface iain/org-todo-keyword-BUY       nil "")
  (defface iain/org-todo-keyword-HOWTO     nil "")
  (defface iain/org-todo-keyword-INFO      nil "")
  (defface iain/org-todo-keyword-COLLECT   nil "")
  (defface iain/org-todo-keyword-SOLVE     nil "")
  (defface iain/org-todo-keyword-READ      nil "")
  (defface iain/org-todo-keyword-READING   nil "")
  (defface iain/org-todo-keyword-PLAN      nil "")


  (setq org-todo-keyword-faces `(
    ("TODO"      . iain/org-todo-keyword-TODO)
    ("DONE"      . iain/org-todo-keyword-DONE)
    ("WAITING"   . iain/org-todo-keyword-WAITING)
    ("CANCELLED" . iain/org-todo-keyword-CANCELLED)
    ("BUY"       . iain/org-todo-keyword-BUY)
    ("HOWTO"     . iain/org-todo-keyword-HOWTO)
    ("INFO"      . iain/org-todo-keyword-INFO)
    ("COLLECT"   . iain/org-todo-keyword-COLLECT)
    ("SOLVE"     . iain/org-todo-keyword-SOLVE)
    ("READ"      . iain/org-todo-keyword-READ)
    ("READING"   . iain/org-todo-keyword-READING)
    ("PLAN"      . iain/org-todo-keyword-PLAN)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; in to account
  (defun iain/org-is-habit-p (&optional pom)
...
#+begin_src emacs-lisp :tangle no
  ...
  (defun iain/force-org-agenda-todo()
    (interactive)
  ...
  (defun iain/org-cancel-todo ()
    (interactive)
    (org-todo "CANCELLED"))

  ;; Break flow, cancel directly, agenda scope
  (defun iain/org-agenda-cancel-todo ()
    (interactive)
...
#+begin_src emacs-lisp :tangle no
  (defun iain/insert-created-timestamp()
...
#+name: org-expiry-createdtimestampusage
#+begin_src emacs-lisp :tangle no
  (defadvice org-insert-todo-heading (after iain/created-timestamp-advice activate)
    "Insert a CREATED property using org-expiry.el for TODO entries"
    (iain/insert-created-timestamp))

  (ad-activate 'org-insert-todo-heading)

  (defadvice org-capture (after iain/created-timestamp-advice activate)
    "Insert a CREATED property using org-expiry.el for TODO entries"
...
#+begin_src emacs-lisp
  ...
  (defun iain/org-align-all-tags ()
    "Wrap org-align-tags to be interactive and apply to all"
    (interactive)
    (org-align-tags t))
  (bind-key "M-\"" 'iain/org-align-all-tags)
#+end_src
...
#+begin_src sh :exports code :tangle ~/bin/ical2org :shebang #!/bin/sh
  ...
  BASEDIR=/home/iain/dat/org/_calendars
  ICAL2ORGPY="/home/iain/.local/bin/ical2orgpy"
  ...
  CALBASE="https://calendars.hsdev.com/iain"
...
#+begin_src emacs-lisp
  ...
  ;; Borrows from https://yiming.dev/blog/2018/03/02/my-org-refile-workflow/
    ...
    (add-to-list 'org-refile-targets
                 '(iain/org-opened-buffers :maxlevel . 10))
...
#+begin_src emacs-lisp
  ...
  - it must be a parent of something already"
    ...
    (and
     ...
     (not (iain/has-DONE-keyword))
     ; must have a child
     (save-excursion (org-goto-first-child))))


  (setq org-refile-target-verify-function 'iain/verify-refile-target)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun iain/skip-non-stuck-projects ()
    ...
    (let* ((subtree-end (save-excursion (org-end-of-subtree t)))
      ...
      (if (and (iain/is-project-p) (not has-next))
          nil ; a stuck project, has subtasks but no next task
        subtree-end)))

  (defun iain/skip-non-projects ()
    "Skip trees that are not projects"
    (let* ((subtree-end (save-excursion (org-end-of-subtree t))))
      (if (iain/is-project-p)
          nil
        subtree-end)))

  (defun iain/skip-projects ()
  ...
  (defun iain/org-remove-empty-propert-drawers ()
  ...
  (defun iain/org-remove-redundant-tags ()
  ...
  (defun iain/org-group-bucket-items (prop items)
    (let ((buckets ()))
      (dolist (item items)
  ...
  (defadvice org-agenda-finalize-entries (around org-group-agenda-finalize
    ...
    (if org-agenda-group-by-property
        ...
        (let ((text ""))
          (dolist (bucket (iain/org-group-bucket-items
  ...
  (defvar iain/org-my-archive-expiry-days 365
    "The number of days after which a completed task should be auto-archived.
   This can be 0 for immediate, or a floating point value.")

  (defun iain/org-my-archive-done-tasks ()
    (interactive)
    (save-excursion
      (goto-char (point-min))
      (let ((done-regexp
            ...
            (when (re-search-forward state-regexp end t)
              (let* ((time-string (match-string 2))
                ...
                (if (>= (time-to-number-of-days
                        ...
                        iain/org-my-archive-expiry-days)
                    (org-archive-subtree)))))))))

  (defalias 'archive-done-tasks 'iain/org-my-archive-done-tasks)
#+end_src
...
#+begin_src emacs-lisp
  (defun iain/insert-key-description ()
...
#+begin_src emacs-lisp
  (use-package selected
    ...
    :bind ( :map selected-keymap           ; bindings for all active regions
            ...
            ; TODO why do these not work in org-journal mode?
...
#+begin_src emacs-lisp
  (defun iain/delete-process-interactive ()
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    :bind ( :map corfu-map
            ("M-m" . iain/corfu-move-to-minibuffer))
    :init
    (global-corfu-mode)

    :config
    ;; In need for more info? Press M-m
    (defun iain/corfu-move-to-minibuffer ()
      (interactive)
...
#+begin_src emacs-lisp
  (use-package tempel
    ...
    :hook ((prog-mode text-mode org-mode) . iain/tempel-setup-capf))
#+end_src
...
#+begin_src emacs-lisp :tangle no
  ...
  (defun iain/pdf-extract-info (file)
  ...
  (defun iain/extended-attach-file (name &rest plist)
...
#+begin_src emacs-lisp
  ...
  (use-package elfeed
    ...
    :bind (("C-c f" . 'elfeed)
           ...
           ("w" . 'iain/elfeed-show-toggle-watchlater)
           ("v" . 'iain/elfeed-play-with-mpv)
           :map elfeed-search-mode-map
           ("w" . 'iain/elfeed-search-toggle-watchlater))
    :init
    (setf url-queue-timeout 30
          elfeed-db-directory "~/.elfeed/")

    :config
    (defun iain/elfeed-search-toggle-tag(tag)
    ...
    (defun iain/elfeed-search-toggle-watchlater()
      (interactive)
      (iain/elfeed-search-toggle-tag 'watchlater))

    (defun iain/elfeed-show-toggle-tag(tag)
      (interactive)
      (if (elfeed-tagged-p tag elfeed-show-entry)
          (elfeed-show-untag tag)
        (elfeed-show-tag tag)))

    (defun iain/elfeed-show-toggle-watchlater()
      (interactive)
      (iain/elfeed-show-toggle-tag 'watchlater))

    ;; umpv maintains a playlist, so adding more videos wil automatically queue
    (defun iain/elfeed-play-with-mpv ()
...
#+begin_src emacs-lisp
  (use-package circe
    :commands (circe iain/chat)
    :custom
    (circe-default-nick "IPmonger")
    (circe-network-options
          `(("libera"
             :host "chat.hsdev.com" :port 6667
             :pass ,(concat "IPmonger@emacs/libera:"
            ...
            ("oftc"
             :host "chat.hsdev.com" :port 6667
             :pass ,(concat "IPmonger@emacs/oftc:"
                            (password-store-get "chat.hsdev.com"))
             :channels ("#osm"))))
    ...
    (set-face-attribute 'circe-prompt-face   nil :foreground iain/cursor-color :height 1.3)
    ...
    (add-hook 'lui-mode-hook 'iain/lui-setup)

    ;; Set the prompt properly on entering the chat mode
    (defun iain/set-circe-prompt ()
      (lui-set-prompt (propertize "➜ " 'face 'circe-prompt-face)))
    (add-hook 'circe-chat-mode-hook 'iain/set-circe-prompt)
...
#+begin_src emacs-lisp
  (use-package lsp-mode
    ...
    (lsp-prefer-capf t)
    (lsp-idle-delay 0.5 "lsp idle tuning for frustration"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package prog-mode
    ...
    :hook ((prog-mode                     ; all modes that derive from prog-mode
            ...
            org-mode org-journal-mode) . iain/prettify-symbols)
    :init

    ;; TODO this is a bit messy, do this in mode packages and per mode
    (defun iain/prettify-symbols ()
      (interactive)
...
#+begin_src emacs-lisp
  ...
  (use-package treesit
    ...
    (treesit-extra-load-path
     '("/home/iain/dat/src/emacs/tree-sitter-module/dist/"))
    :config
...
My personal style will be called =iain= and basing it on the =linux= style.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package scheme
    :custom
    (scheme-program-name "/home/iain/.guix-profile/bin/guile"))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package geiser-guile
      ...
      :demand t
      :hook ((geiser-repl-mode . iain/prettify-symbols))

      :custom
      (geiser-active-implementations '(guile))
      (geiser-guile-binary "/home/iain/.guix-profile/bin/guile")
...
#+begin_src emacs-lisp
  (use-package sly
    ...
    (sly-lisp-implementations
     '((sbcl ("sbcl" "--core" "/home/iain/.sbcl/sbcl.core-for-sly"))))
...
#+begin_src emacs-lisp
  (use-package git-auto-commit-mode
    ...
    :init
    (defun iain/git-auto-commit-mode ()
      (interactive)
